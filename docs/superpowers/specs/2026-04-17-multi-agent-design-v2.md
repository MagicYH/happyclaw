...
| 13 | 并发模式 | 同 folder 完全串行 | **writer/advisor 分级并发**：writer 同 folder 串行；advisor 禁写项目目录但可写 scratch/tmp，容器模式走 `:ro` OS 级强制，宿主机模式走 SDK PreToolUse hook 软强制 | 用户反馈 |
...
6. **writer/advisor 分级并发**：writer 类 Bot 在同 folder 内串行执行；advisor 类 Bot 禁止修改项目目录（但可写 scratch / `/tmp`），可与任何 writer 及其他 advisor 并行
...
- `concurrency_mode` 决定该 Bot 在 folder 队列里的竞争语义（详见 §5.6）：
  - `writer`（默认）：可读写项目目录，同 folder 严格串行
  - `advisor`：**不可修改项目目录**（但仍可写 scratch、`/tmp` 等目录），可与任何其他 agent 并行
...
|------|-----------|--------|---------|
| 项目目录 | `/workspace/group` ← `data/groups/{folder}/` | rw | **禁写** |
| Scratch 目录 | `/workspace/scratch` ← `data/scratch/{folder}/bots/{botId}/` | rw | **rw** |
| Bot 角色 | `/workspace/bot-profile` ← `data/bot-profiles/{botId}/` | rw | rw |
| 临时目录 | `/tmp` | rw | rw |
| SDK session | `/home/node/.claude/` ← `data/sessions/{folder}/bots/{botId}/.claude/` | rw | rw |

**关键点**：
- advisor 有 `/workspace/scratch`（per-bot 持久 scratch，跨会话保留），写 review 笔记、分析文档、中间结果
- `/tmp` 对两种模式都是完全可写（容器内独立的 tmpfs 或宿主机进程的默认 /tmp）
- session 目录必须可写（SDK 需要维护状态），advisor 也不例外

#### 5.6.2 双模式强制策略

**容器模式（OS 级硬保护）**：

- `container-runner.ts` 启动 advisor 容器时：`/workspace/group` 挂载加 `:ro` 标志
- Agent 通过任何方式（Write tool、Bash `>`、Python `open(w)`）尝试写项目目录 → 内核返回 EROFS
- 这是真正的 OS 级保护，无法绕过

**宿主机模式（SDK Hook 软保护）**：

- 宿主机模式下 agent 进程拥有用户的完整文件权限，无法用文件系统权限做隔离
- 改用 Claude Agent SDK 的 **PreToolUse Hook**（基础设施已存在，现有 PreCompact Hook 可参考）
- agent-runner 在启动时根据 `HAPPYCLAW_BOT_MODE=advisor` 注册一个路径守卫 hook，拦截三类调用：
  1. `Write` / `Edit` / `NotebookEdit`：解析目标路径，若位于 `data/groups/{folder}/` 下（宿主机模式）或 `/workspace/group` 下（均指项目目录）则拒绝，放行 `/workspace/scratch`、`/tmp`、session 目录等；返回明确错误信息引导 agent 改写 scratch
...
- Prompt 兜底：advisor 的 system prompt 明确说明："你以只读方式访问项目目录，可以写 `/tmp` 和 `/workspace/scratch`，不要尝试修改项目文件；如需落盘分析结果请写到 scratch"

**宿主机 advisor 的诚实声明**：

- SDK Hook + Prompt 是"best-effort"，不是 OS 级隔离
- Agent 有意/无意通过 subprocess、非标准工具、或未被 hook 覆盖的路径绕过守卫仍有可能修改项目
- admin 主容器（host mode）的信任边界本来就是用户自己，用 advisor 更多是"行为约束提示"而非"安全隔离"
- member 主容器强制 container mode，天然有 `:ro` 硬保护，**不存在此限制**
- 宿主机 advisor 的绕过防御（subprocess 拦截、seccomp profile 等）超出本期范围

#### 5.6.3 其他落地约束

1. **容器环境变量** `HAPPYCLAW_BOT_MODE=writer|advisor` 传给 agent-runner，用于：Hook 初始化、Prompt 注入、系统行为分支
2. **scratch 目录自动创建**：`container-runner.ts` 启动前 `mkdirSync` 确保 `data/scratch/{folder}/bots/{botId}/` 存在
3. **配额**：advisor 和 writer 共享 `MAX_CONCURRENT_CONTAINERS / MAX_CONCURRENT_HOST_PROCESSES` 全局池，per-user 配额也统一计算。advisor 并发只是解除了 folder 级串行约束，不突破全局上限
4. **为什么不做 writer 之间的并发**：writer 真正并发的代价（文件冲突、git 状态混乱、工具调用串扰）需要 worktree / 锁协议等基础设施支撑，本期不做；§10 中明确列为 non-goal
...
同一 folder 下所有 Bot 的 Agent 实例挂载相同的项目目录（`data/groups/{folder}/`）：

- **writer 模式**：项目目录读写（`rw`），folder 级串行队列保证同一时刻只有一个 writer 在写
- **advisor 模式**：项目目录**禁写**，同时拥有独立的 scratch 目录（`/workspace/scratch`）和 `/tmp` 可写
  - 容器模式：`:ro` 挂载项目目录，OS 级硬保护
  - 宿主机模式：SDK PreToolUse Hook 拦截，软保护（诚实声明见 §5.6.2）

详见 §5.6 并发控制（含完整读写矩阵和双模式强制策略）。
...
`data/groups/{folder}/downloads/{channel}/` 按日期 + channel 分层已足够隔离，不按 Bot 再切。文件下载由 writer agent 主动操作（advisor 禁写项目目录，但仍可把下载结果存到 scratch），writer 之间的 folder 级串行队列保证无并发冲突。
...
**设计理由 — 为什么放在项目目录外**：
- 如果放在 `data/groups/{folder}/scratch/`，物理上会落在项目工作区内，`:ro` 挂载会导致 scratch 同时是只读，需要特殊重新挂载处理，容易出错
- 独立的 `data/scratch/` 路径在容器和宿主机两种模式下行为一致，不与项目 git 树交叉
- 和现有 `data/sessions/`、`data/ipc/` 的分离约定一致
...
```
data/
├── groups/{folder}/         ← 项目工作区（挂载到 /workspace/group）
│   ├── CLAUDE.md
│   ├── src/
│   ├── logs/bots/{botId}/   ← 容器日志（系统路径，file-manager 保护）
│   └── downloads/...
├── scratch/{folder}/bots/{botId}/   ← advisor/writer scratch（挂载到 /workspace/scratch）
...
- **宿主机模式 advisor 的绕过防御**：本期依赖 SDK PreToolUse Hook + Prompt 兜底，属于"best-effort"。通过 subprocess、未被 hook 覆盖的工具路径绕过守卫的加固（seccomp profile、subprocess 拦截、chroot 等）不在本期范围；member 主容器强制 container 模式，不受此限制
...
| `src/container-runner.ts` | 挂载 `/workspace/bot-profile`；挂载 `/workspace/scratch`；IPC/logs 路径按 bot_id 切分；`customSystemPrompt` 注入；**advisor 容器模式 `/workspace/group` 挂为 `:ro`**；注入 `HAPPYCLAW_BOT_MODE` 环境变量 |
...
| `container/agent-runner/src/index.ts` | 读取 `HAPPYCLAW_BOT_MODE`；advisor 模式注册 PreToolUse Hook（宿主机模式的路径守卫） |
| `container/agent-runner/src/advisor-guard.ts` | **新增**：PreToolUse Hook 实现，拦截 Write/Edit/NotebookEdit/Bash 的项目目录写入 |
...
- `concurrency-mode.test.ts` — writer 同 folder 串行；advisor 并行启动
- `advisor-guard.test.ts` — 容器模式 `:ro` 挂载写入失败（EROFS）；宿主机模式 PreToolUse Hook 拦截 Write/Edit/Bash `>` 到项目目录，放行 scratch 和 `/tmp`
