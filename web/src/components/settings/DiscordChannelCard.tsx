...
export function DiscordChannelCard() {
  ...
  const loadConfig = useCallback(async () => {
    setLoading(true);
    try {
      const data = await api.get<UserDiscordConfig>(
        '/api/config/user-im/discord',
      );
      setConfig(data);
      setBotToken('');
    } catch {
  ...
  const handleToggle = async (newEnabled: boolean) => {
    setToggling(true);
    try {
      const data = await api.put<UserDiscordConfig>(
        '/api/config/user-im/discord',
        { enabled: newEnabled },
      );
      setConfig(data);
  ...
  const handleTest = async () => {
    setTesting(true);
    try {
      const result = await api.post<DiscordTestResult>(
        '/api/config/user-im/discord/test',
      );
      if (result.success) {
        toast.success(
          `Discord 连接成功！Bot: ${result.bot_name} (@${result.bot_username})`,
        );
      } else {
          ...
          <>
            ...
            <div>
              ...
              <Input
                ...
                placeholder={
                  config?.hasBotToken ? '留空不修改' : '输入 Discord Bot Token'
                }
              />
            </div>
              ...
              <Switch
                ...
                onCheckedChange={async (checked) => {
                  try {
                    ...
                    setConfig((prev) =>
                      prev
                        ? { ...prev, streamingMode: checked ? 'edit' : 'off' }
                        : prev,
                    );
                    toast.success(
                      checked ? '已开启流式编辑' : '已关闭流式编辑',
                    );
                  } catch {
                    toast.error('更新失败');
                  }
            ...
            <div className="text-xs text-muted-foreground mt-2 space-y-1">
              <p>
                获取 Bot Token：Discord Developer Portal → Applications → Bot →
                Token
              </p>
              <p>
                斜杠命令首次注册后通过 Discord Global Commands 通道分发，
                Discord 官方说明可能需要 5-60
                分钟才在所有服务器生效。期间可以用纯文本命令兜底， 例如直接发送{' '}
                <code>/clear</code>、<code>/list</code>、<code>/status</code>。
              </p>
            </div>
          </>
        )}
