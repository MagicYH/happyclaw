      ...
      <Card className="divide-y divide-border overflow-hidden">
        {invites.length === 0 ? (
          ...
          invites.map((invite) => {
            ...
            const isUsedUp =
              invite.max_uses > 0 && invite.used_count >= invite.max_uses;
            return (
              <div
                key={invite.code}
              ...
              >
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {isExpired && (
                      <span className="text-xs px-1.5 py-0.5 bg-error-bg text-error rounded">
                        已过期
                      </span>
                    )}
                    {isUsedUp && (
                      <span className="text-xs px-1.5 py-0.5 bg-orange-100 dark:bg-orange-900/40 text-orange-600 dark:text-orange-400 rounded">
                        已用完
                      </span>
                    )}
                  </div>
                  <div className="text-xs text-muted-foreground mt-1">
                    创建者: {invite.creator_username} · 使用:{' '}
