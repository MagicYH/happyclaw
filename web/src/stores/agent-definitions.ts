...
export const useAgentDefinitionsStore = create<AgentDefinitionsState>(
  (set, get) => ({
    agents: [],
    loading: false,
    error: null,
    ...
    loadAgents: async () => {
      set({ loading: true });
      try {
        const data = await api.get<{ agents: AgentDefinition[] }>(
    ...
    getAgentDetail: async (id: string) => {
      const data = await api.get<{ agent: AgentDefinitionDetail }>(
