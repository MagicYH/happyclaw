...
interface WorkspaceConfigState {
  ...
  updateWorkspaceMcp: (
    jid: string,
    id: string,
    updates: Partial<WorkspaceMcpServer>,
  ) => Promise<void>;
  toggleWorkspaceMcp: (
    jid: string,
    id: string,
...
export const useWorkspaceConfigStore = create<WorkspaceConfigState>(
  (set, get) => ({
    ...
    loadWorkspaceSkills: async (jid) => {
      ...
      try {
        const data = await api.get<{ skills: WorkspaceSkill[] }>(
          `${groupBase(jid)}/skills`,
        );
    ...
    toggleWorkspaceSkill: async (jid, id, enabled) => {
      try {
        await api.patch(`${groupBase(jid)}/skills/${encodeURIComponent(id)}`, {
          enabled,
        });
        await get().loadWorkspaceSkills(jid);
      } catch (err) {
    ...
    loadWorkspaceMcp: async (jid) => {
      ...
      try {
        const data = await api.get<{ servers: WorkspaceMcpServer[] }>(
          `${groupBase(jid)}/mcp-servers`,
        );
    ...
    updateWorkspaceMcp: async (jid, id, updates) => {
      try {
        await api.patch(
          `${groupBase(jid)}/mcp-servers/${encodeURIComponent(id)}`,
          updates,
        );
        set({ mcpError: null });
        await get().loadWorkspaceMcp(jid);
